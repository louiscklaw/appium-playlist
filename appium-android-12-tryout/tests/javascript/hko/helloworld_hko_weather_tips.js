...
const { TOP_LEFT_MENU_BUTTON, MENU_BUTTON_WEATHER_TIPS, TOP_LEFT_BACK_BUTTON } = require('./utils/XPATHS');
...
async function main() {
  ...
  try {
    ...
    element = await driver.$(MENU_BUTTON_WEATHER_TIPS);
    await element.click();

    await driver.pause(1 * 1000);

    screenshot_raw = await driver.takeScreenshot();
    fs.writeFileSync(`${SCREENCAPTURE_DIR}/weather_tips_result.png`, screenshot_raw, { encoding: 'base64' });
  } catch (error) {
